...
PUBLISHED_DATA_DIR=data-processing/data/published
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
    if [ "${1:-}" = "--help" ]; then
        ...
        if git diff --quiet -- ${PUBLISHED_DATA_DIR}; then
        ...
        if git ls-remote --exit-code origin ${TARGET_BRANCH}; then
            ...
            if git diff --quiet HEAD origin/${TARGET_BRANCH} -- ${PUBLISHED_DATA_DIR}; then
        ...
        if ! git diff --quiet -- ${PUBLISHED_DATA_DIR}; then
            ...
            if ! git ls-remote --exit-code origin ${TARGET_BRANCH} || \
                  ! git diff --quiet HEAD origin/${TARGET_BRANCH} -- ${PUBLISHED_DATA_DIR}; then
